...
export class ConnectionManager {
  constructor() {
    // 连接配置
    this.config = {
      minInputLength: 5,           // 触发预连接的最小输入长度
      typingIdleDelay: 1000,       // 输入停顿多久后触发预连接 (毫秒)
      connectionTimeout: 30000,    // 连接超时时间 (30秒)
      closeDelay: 10000,           // 使用后延迟关闭时间 (10秒)
      maxPoolSize: 1,              // 连接池大小 (通常为1)
      frequentUseThreshold: 3,     // 频繁使用阈值 (次数/分钟)
      frequentUserTimeout: 120000, // 频繁用户的连接保持时间 (2分钟)
    };
  ...
  releaseConnection(success = true) {
    ...
    console.log(`连接已释放，将在 ${closeDelay/1000} 秒后关闭（如果未再次使用）`);
  }
  ...
  closeConnection(force = false) {
    ...
    // 尝试通知服务器关闭连接
    fetch(`/api/close-connection/${this.activeConnection.sessionId}`, {
      method: 'POST',
      headers: {
